...
function PageLayout({ leftImage, rightImage, content, imageClass = "" }) {
  ...
  const mdComponents = {
    h1: ({ children }) => (
      <p className="text-5xl font-mono text-gray-900 dark:text-white">{children}</p>
    ),
    h2: ({ children }) => (
    ...
    h4: ({ children }) => (
      <p className="text-2xl font-bold font-mono text-gray-900 dark:text-white">{children}</p>
    ),
    h5: ({ children }) => (
      <p className="text-[21px] font-mono text-gray-900 dark:text-white">{children}</p>
    ),
    p: ({ children }) => <p className="font-light">{children}</p>,
    img: ({ src, alt }) => (
      <img src={src} alt={alt} className="w-[68%] rounded-2xl mx-auto block border-2" />
    ),
    pre: ({ children }) => (
  ...
  return (
    <div className="flex-1 flex w-full" style={{ minHeight: 'calc(100vh)' }}>
      ...
      <div className="flex-shrink-0 w-[800px] px-[50px] pb-6 font-sans flex flex-col justify-end text-[22px] relative z-10 text-gray-900 dark:text-gray-100">
